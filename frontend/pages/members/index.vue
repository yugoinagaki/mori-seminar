  ...
  <div>
    ...
    <div class="bg-white border-b border-gray-100 sticky top-[60px] z-40">
      <div class="max-w-7xl mx-auto px-6 md:px-14 flex gap-0">
        <button
          v-for="tab in [{ value: 'active', label: '現役メンバー' }, { value: 'ob_og', label: 'OB・OG' }]"
          :key="tab.value"
          class="px-6 py-4 text-sm font-medium border-b-2 transition-colors"
          :class="activeTab === tab.value ? 'border-primary-700 text-primary-700' : 'border-transparent text-gray-400 hover:text-gray-700'"
          @click="activeTab = tab.value as 'active' | 'ob_og'"
        >
          {{ tab.label }}
        </button>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-14 py-16">
        ...
        <div v-for="member in filtered" :key="member.id" class="group">
          <div class="aspect-square overflow-hidden mb-3 bg-gray-100">
            ...
            <div v-else class="w-full h-full bg-primary-50 flex items-center justify-center">
              <span class="text-primary-300 text-3xl font-bold">{{ member.name[0] }}</span>
            </div>
          </div>
          ...
          <p v-if="member.major" class="text-xs text-gray-400">{{ member.major }}</p>
          <p v-if="member.bio" class="text-xs text-gray-500 mt-2 leading-relaxed line-clamp-3">{{ member.bio }}</p>
          <div class="flex gap-3 mt-2">
            <a v-if="member.twitter_url" :href="member.twitter_url" target="_blank" rel="noopener"
              class="text-gray-300 hover:text-primary-700 transition-colors">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </a>
            <a v-if="member.instagram_url" :href="member.instagram_url" target="_blank" rel="noopener"
              class="text-gray-300 hover:text-primary-700 transition-colors">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <p v-else class="py-16 text-center text-gray-400">メンバーが登録されていません</p>
    </div>
  </div>
</template>
