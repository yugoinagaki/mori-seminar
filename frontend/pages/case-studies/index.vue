...
const params = computed(() => ({ per_page: 12, page: page.value }))
const { data, refresh } = await get<{ data: any[]; last_page: number }>('/case-studies', params.value)
  ...
  <div>
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-14 py-16">
      <div v-if="data?.data?.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <NuxtLink
          v-for="cs in data.data" :key="cs.id"
          :to="`/case-studies/${cs.slug}`"
          class="fade-in group block"
        >
          <div class="aspect-video bg-primary-50 overflow-hidden mb-4">
            <img v-if="cs.thumbnail_url"
              :src="useStorageUrl(cs.thumbnail_url) ?? ''"
              :alt="cs.title"
          ...
          <p v-if="cs.description" class="text-gray-500 text-sm mt-2 leading-relaxed line-clamp-2">
            {{ cs.description }}
          </p>
          <p v-if="cs.author" class="text-gray-400 text-xs mt-2">{{ cs.author.name }}</p>
        </NuxtLink>
      </div>
      <p v-else class="py-16 text-center text-gray-400">まだケーススタディがありません</p>
