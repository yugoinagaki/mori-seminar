...
<div>
    ...
    <div class="max-w-2xl mx-auto px-6 md:px-14 py-20">
        ...
        <div class="space-y-10">
            ...
            @if($contactInfo->contact_twitter_url || $contactInfo->contact_instagram_url)
            <div>
                <p class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-700 mb-4">SNS</p>
                <div class="flex gap-5">
                    @if($contactInfo->contact_twitter_url)
                    <a href="{{ $contactInfo->contact_twitter_url }}" target="_blank" rel="noopener"
                    ...
                    @endif
                    @if($contactInfo->contact_instagram_url)
                    <a href="{{ $contactInfo->contact_instagram_url }}" target="_blank" rel="noopener"
                       class="flex items-center gap-2 text-gray-600 hover:text-primary-700 transition-colors text-sm">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            @if($contactInfo->isPageVisible('faq'))
