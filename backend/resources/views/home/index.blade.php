...
@section('title', '森聡研究会 | 慶應義塾大学')

@section('content')
@php
$heroImageUrl = storage_url($settings->hero_image_url ?? null);
$typeStyle = [
...
<div class="overflow-x-hidden">
    ...
    <section class="relative min-h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div class="absolute inset-0">
            @if($heroImageUrl)
            <img src="{{ $heroImageUrl }}" alt="" class="w-full h-full object-cover object-center" />
            @else
            <div class="w-full h-full"
                 style="background: linear-gradient(155deg, #041c33 0%, #083a63 45%, #0d5189 100%)"></div>
            @endif
        </div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-14 pt-36 pb-28">
            <div class="text-center md:text-left">
                ...
                <h1 class="text-white font-bold mb-4 leading-none font-mincho"
                    ...
                    @foreach(mb_str_split($heroText) as $i => $char)
                    <span class="hero-char" style="animation-delay: {{ number_format(0.2 + $i * 0.2, 1) }}s">{{ $char }}</span>
                    @endforeach
                </h1>
                ...
                <div class="hero-sub flex flex-wrap justify-center md:justify-start gap-4"
                     style="animation-delay: 2.1s">
                    <a href="/theme" class="btn-outline-white">
                        研究テーマについて
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                        </svg>
                    </a>
                    <a href="/professor" class="btn-outline-white">
                        森聡教授について
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                        </svg>
        ...
        {{-- E. 右上ニュースカード (デスクトップのみ) --}}
        @if(count($worldNews))
        <div class="hidden lg:block fixed top-28 right-4 w-64 z-40 hero-sub" style="animation-delay: 2.6s" id="news-card-wrapper">
            {{-- 非表示時の「Newsを表示」ボタン --}}
            <button
                id="news-card-show-btn"
                ...
                style="background: rgba(4, 18, 35, 0.7); display: none"
            >
            ...
            {{-- カード上のマイナスボタン --}}
            <div class="flex justify-end mb-1" id="news-card-close-wrapper">
                <button
                    id="news-card-close-btn"
            ...
            >
                <div class="flex items-center justify-between px-4 py-2.5 border-b border-white/10">
                    ...
                    <span id="news-card-counter" class="text-white/25 text-[9px] font-mono">1 / {{ count($worldNews) }}</span>
                </div>

                <div class="px-4 py-4">
                    <p id="news-card-title" class="text-white/85 text-sm leading-relaxed group-hover:text-white transition-colors font-medium">
                        {{ $worldNews[0]['title'] ?? '' }}
                    </p>
                    <p id="news-card-time" class="text-white/30 text-[10px] font-mono mt-3"></p>
                </div>

                <div class="h-px bg-white/10">
                    <div id="news-card-progress" class="h-full bg-primary-400 transition-none" style="width: 0%"></div>
                </div>

                <div class="flex items-center justify-between px-4 py-2.5">
                    <div class="flex gap-1" id="news-card-dots"></div>
    ...
    <div class="bg-primary-700 py-2.5 overflow-hidden select-none">
        <div class="ticker-track">
            @foreach(range(0, 1) as $pass)
            @foreach($tickerPosts as $item)
            <a
                href="/news/{{ $item->slug }}"
                class="text-white/80 hover:text-white text-xs tracking-wide font-light mx-10 whitespace-nowrap transition-colors"
            >
                <span class="text-white/30 mr-6">◆</span>{{ $item->published_at?->format('Y.m.d') }} ── {{ $item->title }}
            </a>
            @endforeach
            @endforeach
        </div>
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-6 md:px-14">
            <div class="flex items-end justify-between mb-12">
                ...
                <a href="/news" class="hidden md:inline-flex items-center gap-2 text-primary-700 text-sm font-medium hover:gap-3 transition-all fade-in">
                    一覧を見る
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                    </svg>
    ...
    <section class="py-28 relative overflow-hidden"
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-14 fade-in">
            <div class="max-w-2xl">
                ...
                @if($theme->content)
                <div class="text-white/65 text-sm md:text-base leading-[2] mb-10 line-clamp-3">{!! $theme->content !!}</div>
                @endif
                <a href="/theme"
            ...
            <div class="grid md:grid-cols-2 gap-16 lg:gap-24 items-center">
                <div class="fade-in relative">
                    <div class="aspect-[4/5] overflow-hidden bg-primary-100">
                        <img
                            src="{{ storage_url($professor->profile_image_url) ?? 'https://picsum.photos/seed/professor/600/750' }}"
                            alt="{{ $professor->name }}"
                ...
                <div class="fade-in">
                    ...
                    @if($professor->bio)
                    <div class="text-gray-500 text-sm leading-[2] mb-5 line-clamp-4">{!! $professor->bio !!}</div>
                    @endif
                    ...
                    @endif
                    <a href="/professor" class="btn-primary">
                        プロフィールを見る
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                        </svg>
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-6 md:px-14">
            <div class="flex items-end justify-between mb-12">
                ...
                <a href="/members" class="hidden md:inline-flex items-center gap-2 text-primary-700 text-sm font-medium hover:gap-3 transition-all fade-in">
                    全員を見る
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                    </svg>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-5">
                ...
                <div class="fade-in group cursor-pointer">
                    <div class="aspect-square overflow-hidden mb-3 bg-gray-100">
                        ...
                        <img src="{{ storage_url($member->profile_image_url) }}"
                             alt="{{ $member->name }}"
    ...
    <section class="py-28 relative overflow-hidden"
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-14 text-center fade-in">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <a href="/faq" class="inline-flex items-center justify-center gap-2 px-7 py-3.5 text-white/40 hover:text-white text-sm font-medium transition-colors tracking-wide">
                    FAQ を見る
                </a>
            </div>
        </div>
